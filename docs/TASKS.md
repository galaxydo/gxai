...
- [ ] **Advanced Tool Auth Hooks** — Allow developers to inject an asynchronous authorization callback directly into `MCPTool` definitions, letting host applications intercept and optionally reject tool invocations *before* execution starts, rather than just returning error strings after.
- [ ] **Agent State Serialization** — Add `.toJSON()` and `.fromJSON()` static methods to `LoopAgent` to serialize the entire message and tagStack history, allowing for seamless suspension and resumption of long-running agent threads across Node process restarts without losing conversation boundary context.
